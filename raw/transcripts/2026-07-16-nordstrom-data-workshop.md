# Nordstrom — data team workshop

**Date:** 2026-07-16
**Type:** Transcript (edited)
**Attendees:** Fatima Al-Rashid (Head of Data), Oliver Grant (Director, Platform Engineering),
Priya Raman (SE), Josefina Van Thienen
**Duration:** 90 min

---

**Oliver:** The purpose today is for Fatima's team to pressure-test the migration proposal.

**Fatima:** I'll be direct. I don't think we should do this, and I want to explain why rather
than just blocking it.

**Josefina:** Please.

**Fatima:** Three things. First, my team knows the incumbent tool. Retraining eight analysts
costs us a quarter of productivity. Second, we have four years of saved queries and dashboards
that would need rebuilding. Third — and this is the one that actually worries me — the
incumbent has a feature for incremental materialization that we depend on daily, and I haven't
seen an equivalent.

**Priya:** On the third one, there is an equivalent. It's just named differently. I can show you.

**Fatima:** Then show me. If that's wrong, one of my three objections goes away.

**Oliver:** What about the first two?

**Fatima:** Those are real costs. I'm not saying they're disqualifying. I'm saying nobody has
put a number on them and the proposal reads as though they're zero.

**Josefina:** That's fair.

**Fatima:** I'd also point out we committed to a two-year agreement and we're under-consuming it.
Adding more scope to a commitment we're not using feels backwards.

**Oliver:** That's a separate conversation.

**Fatima:** It's the same conversation from where I sit.

## Actions

- Priya: demonstrate the materialization equivalent
- Josefina: put actual numbers on retraining and rebuild costs
- Oliver: raise the under-consumption question with finance

## My read

Fatima is not a blocker in the political sense. She's a serious technical evaluator with three
specific objections, and one of them is factually wrong and fixable.

**The under-consumption comment is the one I keep thinking about.** They committed, they're
behind on drawdown, and it's now making them *more* reluctant to expand. The commitment is
working against us.
